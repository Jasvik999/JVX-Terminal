"""
JVX-BlackMagic Hybrid Terminal v31.0 - PROFESSIONAL EDITION
Features: Login, Portfolio, Options Chain, Order Book, Backtest, 
          News, Trade Ideas, Alerts, Sentiment, Economic Calendar, AI Assistant
"""
...
<style>
    ...
    .metric-card { background: #fff; border: 1px solid #e0e0e0; }
    .idea-card { background: #fff; border: 1px solid #e0e0e0; }
    .news-card { background: #fff; border-left: 3px solid #00d4aa; }
    .news-negative { border-left-color: #ff1744 !important; }
    .news-neutral { border-left-color: #ff9100 !important; }
    .ticker-wrap { background: #fff; border: 1px solid #e0e0e0; }
</style>
"""

st.set_page_config(page_title="JVX Hybrid Terminal v31.0", layout="wide", page_icon="📈")
    ...
    with c2:
        st.markdown("""
            <div class="fade-in" style="text-align: center;">
                ...
                <p style="color: #888; letter-spacing: 2px;">TERMINAL v31.0 // PRO EDITION</p>
            </div>
...
def simulate_market_tick():
    ...
    for alert in st.session_state.alerts:
        if not alert['triggered']:
            ...
            if (alert['condition'] == 'ABOVE' and ltp >= alert['price']) or (alert['condition'] == 'BELOW' and ltp <= alert['price']):
                alert['triggered'] = True
                alert['trigger_time'] = datetime.now().strftime("%H:%M:%S")
                st.session_state.alert_log.append(f"🔔 {sym} {alert['condition']} ₹{alert['price']} @ {alert['trigger_time']}")

def get_market_depth(ltp):
    tick = 0.05
    bids = pd.DataFrame({'Bid Qty': np.random.randint(200, 5000, 5), 'Bid Price': [round(ltp - (i*tick), 2) for i in range(1, 6)]})
    asks = pd.DataFrame({'Ask Price': [round(ltp + (i*tick), 2) for i in range(1, 6)], 'Ask Qty': np.random.randint(200, 5000, 5)})
    return bids, asks
...
# Theme toggle
st.session_state.theme = st.sidebar.selectbox("🌗 Theme", ["dark", "light"], index=0 if st.session_state.theme=='dark' else 1)
...
if menu_choice == "💡 Trade Ideas":
    ...
    m2.metric("BUY Setups", buy_count)
    m3.metric("SELL Setups", sell_count)
...
# --- MARKET DEPTH ---
    ...
    with col1:
        ...
        st.dataframe(scan_df.style.map(color_signal, subset=['Signal']), use_container_width=True, hide_index=True)
        selected_sym = st.selectbox("🔍 Select Symbol:", st.session_state.watchlist)
    with col2:
        ...
        st.markdown(f"""<div style="text-align: center; margin-top: 15px; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px;"><span style="color: #888;">Spread: </span><span style="color: #00d4aa; font-weight: bold; font-size: 1.1em;">₹{spread}</span></div>""", unsafe_allow_html=True)
...
# --- OPTIONS CHAIN ---
    ...
    def highlight_atm(row):
        if row['Strike'] == round(row['Spot'] / 50) * 50 if "NIFTY" in opt_sym else round(row['Spot'] / 100) * 100:
...
# --- ORDER BOOK ---
    ...
    if st.session_state.orders:
        ...
        # Simulate order execution on tick
...
# --- SENTIMENT ---
    ...
    c1.markdown(f"""
        <div class="metric-card">
            <div style="font-size: 0.8em; color: #888;">Fear & Greed</div>
            <div style="font-size: 2em; font-weight: bold; color: {fg_color};">{fear_greed}</div>
            <div style="font-size: 0.75em; color: {fg_color};">{"GREED" if fear_greed > 60 else "NEUTRAL" if fear_greed > 40 else "FEAR"}</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    c2.markdown(f"""
        <div class="metric-card">
            <div style="font-size: 0.8em; color: #888;">Put/Call Ratio</div>
            <div style="font-size: 2em; font-weight: bold; color: {pcr_color};">{pcr}</div>
            <div style="font-size: 0.75em; color: {pcr_color};">{"Bullish" if pcr < 1 else "Bearish"}</div>
        </div>
    """, unsafe_allow_html=True)
    
    c3.markdown(f"""
        <div class="metric-card">
            <div style="font-size: 0.8em; color: #888;">Advance-Decline</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #00c853;">{advance}</div>
            <div style="font-size: 1.5em; font-weight: bold; color: #ff1744;">{decline}</div>
            <div style="font-size: 0.75em; color: #888;">Ratio: {round(advance/decline, 2)}</div>
        </div>
    """, unsafe_allow_html=True)
    
    c4.markdown(f"""
        <div class="metric-card">
            <div style="font-size: 0.8em; color: #888;">India VIX</div>
            <div style="font-size: 2em; font-weight: bold; color: #ff9100;">{vix}</div>
            <div style="font-size: 0.75em; color: #888;">Volatility Index</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    for ev in events:
        ...
        st.markdown(f"""
            <div class="idea-card fade-in" style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <div style="color: #888; font-size: 0.8em;">{ev['time']}</div>
                    <div style="color: #fff; font-weight: bold; font-size: 1.1em;">{ev['event']}</div>
                    <div style="color: #666; font-size: 0.8em;">Forecast: {ev['forecast']} | Actual: {ev['actual']}</div>
                </div>
                <div style="background: {impact_color}; color: #000; padding: 4px 12px; border-radius: 4px; font-weight: bold; font-size: 0.8em;">
                    {ev['impact']}
                </div>
            </div>
        """, unsafe_allow_html=True)
...
# --- AI ASSISTANT ---
    ...
    if user_input:
        ...
        if "nifty" in user_lower and "target" in user_lower:
            ...
            response = "General rule: Keep SL at 1.5x ATR from entry. For intraday, max 1% of capital per trade. Never risk more than 2% on any single setup."
        elif "strategy" in user_lower:
            response = f"Currently active: {st.session_state.selected_strategy}. T3 + RSI works best in trending markets. For sideways, switch to EMA + VWAP."
        elif "margin" in user_lower:
            response = f"Your available margin is ₹{st.session_state.balance - st.session_state.used_margin:,.0f}. Used: ₹{st.session_state.used_margin:,.0f}."
        elif "circuit" in user_lower or "breaker" in user_lower:
            response = f"Circuit breaker trips at {st.session_state.loss_limit} consecutive losses. Current streak: {st.session_state.loss_streak}. Take a break if hit."
        elif "option" in user_lower or "call" in user_lower or "put" in user_lower:
            response = "For options, check the Options Chain tab. ATM strikes have highest liquidity. Avoid buying far OTM options with less than 7 DTE."
            ...
            response = "I'm your trading copilot. Ask me about: levels, strategies, risk management, margin, or specific stocks like NIFTY, RELIANCE, etc."
